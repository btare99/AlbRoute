...
// ─── SKELETON COMPONENTS ───────────────────────────────────────────────────────
const sk = (w: string, h: string, r = 8) => (
  <div style={{
...
const SkeletonRouteCard = () => (
  <div style={{ padding: '20px', borderRadius: 12, background: T.surface, border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
...
const SkeletonBusCard = () => (
  <div style={{ padding: '18px', borderRadius: 12, background: T.surface, border: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', gap: 14 }}>
...
const SkeletonTableRow = ({ cols = 5 }: { cols?: number }) => (
  <tr>
...
const SkeletonStatCard = ({ color }: any) => (
  <div style={{ padding: '18px 20px', borderRadius: 12, background: T.surface, border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', gap: 14 }}>
    <div style={{ width: 40, height: 40, borderRadius: 10, background: `${color}14`, flexShrink: 0 }} />
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {sk('60px', '10px', 4)}
      {sk('40px', '20px', 5)}
    </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {isLoadingBuses ? (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 12 }}>
                {Array.from({ length: 6 }).map((_, i) => <SkeletonRouteCard key={i} />)}
              </div>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 12 }}>
                {(currentAccount?.role === 'operator'
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {isLoadingBuses ? (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 12 }}>
                {Array.from({ length: 4 }).map((_, i) => <SkeletonBusCard key={i} />)}
              </div>
            ) : routeBuses.length === 0 ? (

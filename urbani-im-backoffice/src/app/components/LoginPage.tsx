  ...
  return (
        ...
        <div className="login-card">
          ...
          <div className="role-panel" style={{ borderColor: isDispatcher ? '#2d1f5e' : '#0f3d2e' }}>
            ...
            <div className="role-text">
              <strong>{isDispatcher ? 'Hyrje Administrative' : 'Hyrje Stafi'}</strong>
              <small>{isDispatcher ? 'Username + PIN — vetëm Dispatcher.' : 'ID + PIN — Hyrje për të gjithë stafin operacional.'}</small>
            </div>
          ...
          <form onSubmit={handleSubmit} className="login-form">
            ...
            >
              {loading
                ...
                : <>{isDispatcher ? 'Hyr si Dispatcher' : 'Hyr si Staf'}<ArrowRight size={14} /></>
              }
            </button>
          </form>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 600px) {
      .bo-login-shell { overflow-y: auto; }
      .login-right { padding: 0; align-items: stretch; }
      .login-card { 
        width: 100%; 
        min-height: 100vh; 
        border-radius: 0; 
      ...
      .mode-toggle { grid-template-columns: 1fr 1fr; gap: 8px; }
    }
  `}</style>
    </div>
  );

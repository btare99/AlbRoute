    ...
    if (mode === 'staff') {
      try {
        ...
        if (res.ok && data.user) {
          ...
          addNotification(`Mirë se erdhe, ${data.user.name}! 🚌`, 'success');
        } else {
          setError(data.error || 'Username ose PIN i pasaktë.');
        }
      } catch (err) {
        ...
        <div className="card" style={{ padding: '32px' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            {mode !== 'staff' ? (
              ...
              <>
                <div>
                  <label className="label">Përdoruesi (Username)</label>
                  <input className="input-field" type="text" placeholder="p.sh. shoferi.1" value={email} onChange={e => setEmail(e.target.value)} required />
                </div>
                <div>
                  <label className="label">PIN-i i Sigurisë</label>
                  <div style={{ position: 'relative' }}>
            ...
            <button type="submit" className="btn btn-primary" style={{ width: '100%', padding: '13px', marginTop: '4px' }} disabled={loading}>
              {loading ? <span className="animate-spin" style={{ display: 'inline-block', width: '18px', height: '18px', border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%' }}></span>
                : <>{mode === 'login' ? 'Hyr në llogari' : mode === 'register' ? 'Krijo llogarinë' : 'Hyr si Personel'} <ArrowRight size={16} /></>}
            </button>
          </form>
          ...
          {mode === 'staff' && (
            <div style={{ marginTop: '20px', padding: '16px', background: 'rgba(59,130,246,0.05)', borderRadius: '10px', border: '1px solid rgba(59,130,246,0.1)' }}>
              <p style={{ fontSize: '12px', color: '#3b82f6', marginBottom: '4px', fontWeight: '700' }}>UDHËZIM PËR STAFIN:</p>
              <p style={{ fontSize: '11px', color: 'var(--text-muted)', lineHeight: '1.5' }}>Përdorni emrin e përdoruesit dhe PIN-in e dhënë nga administrata për të parë orarin tuaj.</p>
            </div>
          )}
